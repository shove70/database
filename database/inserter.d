...
struct DBInserter(Connection, E : Exception, char quote, alias isValueType, alias appendValue)
{
	...
	private bool tryAppendField(string member, string parentMembers = "", T)(ref const T param, ref size_t fieldHash) {
	...
	void row(T)(ref const T param) if (!isValueType!T) {
		...
		foreach (i, ref fieldHash; fieldsHash) {
			...
			if (!fieldFound)
				throw new E("field '%s' was not found in struct => '%s' members".format(fieldsNames.ptr[i], (Unqual!T).stringof));
